...
OCR - train the model using MNIST data set / test the model using the trained classifier for an image
'''
...
if (dataset_load_method == 'load'):
    X_train = np.load("./MNIST/train_images.npz")['data']
    X_test = np.load("./MNIST/train_labels.npz")['data']
    Y_train = np.load("./MNIST/test_images.npz")['data']
    Y_test  = np.load("./MNIST/test_labels.npz")['data']
'''

#(X_train, Y_train), (X_test, Y_test) = tf.keras.datasets.mnist.load_data()

'''
X_train, X_test = loadlocal_mnist(
        images_path='./MNIST/train-images-idx3-ubyte', 
        labels_path='./MNIST/train-labels-idx1-ubyte')
#(X_train, Y_train), (X_test, Y_test) = tf.keras.datasets.mnist.load_data()

Y_train, Y_test = loadlocal_mnist(
...
X_train_chars=data[:n_samples // 2]
Y_train_chars=digits.target[:n_samples // 2]
...
#X_train_chars , Y_train_chars, X_test_chars, Y_test_chars  = train_test_split(X, Y, test_size=0.20)

print("-------------------")
print(len(X_train_chars))
#print(len(X_test_chars))
print(len(Y_train_chars))
#print(len(Y_test_chars))
#print(X.shape)
#print(Y.shape)
#print(Y[100])
#print(X_train[1])
...
def crop_characters(img, bboxes, n):
    ...
    for i in range(0, n):
        ...
        if (c.shape[0] != 0 and c.shape[1] != 0):
            c = resize(c, (8, 8), mode='constant', cval=1.0, clip=True)
            characters.append((c<0.80).reshape(64))
...
#mlp = MLPClassifier(hidden_layer_sizes=(300,), activation='relu', max_iter=5000, tol=0.0001, random_state=1, verbose=verbose_classifiers)
#
...
for i in range(0, len(X_test)):
   ...
   # characters, charlabels = Y_train, Y_test
    characters = Y_train  
    charlabels = Y_test
...
#if (save_classifiers):
joblib.dump(rbm_mlp_classifier, './MNIST/RBM_MLP_classifier.pkl')


print("finished")

test_image="/home/bl/HNR/Handwritten-Names-Recognition-master/Notebook/test3.png"
...
rbm_mlp_classifier = joblib.load('./MNIST/RBM_MLP_classifier.pkl')

rbm_mlp_prediction = rbm_mlp_classifier.predict(characters)

print(rbm_mlp_prediction)

print(rbm_mlp_classifier.score(X_train_chars, Y_train_chars))
