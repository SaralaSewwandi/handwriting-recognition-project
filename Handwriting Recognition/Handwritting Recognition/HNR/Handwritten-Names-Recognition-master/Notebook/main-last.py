...
#function to create  the full data set & labels
#As the database df we imported doesn't contain the images but link to the corresponding images, we need to download them and create a dataset with those and the labels.
#The delborders function eliminates the bottom border noise in case of "first_b" and "last_b" types in the dataset.
def delborders(crop):
...
#The getcrop function obtains an image from an entry of the database and returns the image in grayscale and cropped to show just the name part.
def getcrop(n):
    try: 
...
#Next we define the gen_dataset function, this function creates a binarized image list "data" and a label list "labels" from the first n entries of the database (if not specified uses all the entries of the database) using the crops of the getcrop function and its corresponding label on that index.
...
#Now we can download and create the whole dataset and its corresponding label list or load the dataset from the saved files.
...
print("***********")
#print(get_characters_img_only("10.png"))
print("**************")
...
str=predict_full_name("10.png",mlp_classifier_HOG)
print(str)
